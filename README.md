...
- Builds a map of the highest version string seen for each dependency using JavaScript's string comparison (lexicographic order). This means `"9.0.0"` is considered higher than `"10.0.0"` in the current implementation, which may not match semver expectations.
